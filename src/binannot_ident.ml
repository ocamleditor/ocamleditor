                ...
                match locations.(i) with
                  ...
                  | Some ({ident_kind; ident_loc; _} as def_ident) ->
                    match ident_kind with
                      ...
                      | Int_ref _ | Ext_ref | Def _ | Def_constr _ | Def_module _ -> ()
              done;
    ...
    | Def_module def ->
      ...
      let mod_refs =
        find_external_references ~project
          ~ext_ref:{ident_kind=Ext_ref; ident_loc=mkloc def.def_name def.def_loc; ident_fname=""}
      in
