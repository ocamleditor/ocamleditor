...
let path = if Oe_config.ocamleditor_debug then "/VERSION_TEST.txt" else "/VERSION.txt"
...
(** parse *)
let parse result inchan =
  try
    while true do
      let line = input_line inchan in
      if Str.string_match re line 0 then begin
        let version = Str.matched_group 1 line in
        if version > Oe_config.version then (result := Some (Str.matched_group 1 line));
        raise End_of_file
      end
    done;
        ...
        begin
          match check (); with
            ...
            | Some ver ->
              ...
              let label = sprintf "A new version of %s is available (%s)" Oe_config.title ver in
