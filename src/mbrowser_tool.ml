...
object (self)
  ...
  method find ?text ?(fill=false) () =
    Timeout.set tout_entry_find begin fun () ->
      ...
      if String.length text > 0 then begin
        ...
        let symbols = Symbol.filter_by_name ~regexp project.Project.symbols.syt_table in
  ...
  method find_compl ~prefix ~(page : Editor_page.page) ~include_methods ?(f=fun _ -> ()) () =
    ...
    if String.length prefix > 0 then begin
      ...
      let symbols = Symbol.filter_by_name
        ...
        ~regexp
        project.Project.symbols.syt_table
      in
  ...
  method private find_symbol ?(root=[]) text =
    ...
    match Symbol.find_by_modulepath ~kind:type_kinds project.Project.symbols ident with
      | None ->
        (* If the symbol is relative, search recursively in the parent module *)
        let rec find ?(root=[]) text =
          let absolute_ident = root @ ident in
          match Symbol.find_by_modulepath ~kind:type_kinds project.Project.symbols absolute_ident with
            | None ->
  ...
  method private create_widget_library ~lib_path ?(f=fun _ -> ()) () =
    ...
    if lib_path = project.Project.ocamllib then
  ...
  method create_widget_module ~module_path ?(lib_path="") ?(sort=false) ?(f=fun _ -> ()) () =
    ...
    Activity.wrap Activity.Symbol begin fun () ->
      let cache_changed = Symbol.Cache.update ~cache:project.Project.symbols ~value_path:module_path () in
      let widget =
        try
        ...
        with Not_found ->
          ...
          let entries = Symbol.filter_by_modulepath ~update_cache:false project.Project.symbols module_path in
        ...
        begin
          match self#get_current_symbol () with
            | Some symbol' (* Symbol under the cursor *) ->
              ...
              in
              begin
                match Symbol.find_parent project.Project.symbols symbol' with
                  | Some parent when current_page#is_search_output -> go symbol' parent;
                  | Some parent ->
                    begin
                      match Symbol.find_parent project.Project.symbols parent with
