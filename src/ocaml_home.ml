...
class widget ~project ?(spacing=3) ?border_width ~label_width ?packing ()=
  ...
  let _ = GMisc.label ~xalign:0.0 ~width:label_width ~text:"OCAMLLIB:" ~show:is_win32 ~packing:(table#attach ~top:!top ~left:0) () in
  let box = GPack.hbox ~spacing ~packing:(table#attach ~top:!top ~left:1 ~expand:`X) () in
  let entry_ocamllib = GEdit.entry ~editable:true ~show:is_win32 ~packing:box#add () in
  let button_ocamllib = GButton.button ~label:"  ...  " ~show:is_win32 ~packing:box#pack () in
