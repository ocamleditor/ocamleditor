...
let file ~browser ~group ~flags  () =
  ...
  save_file#connect#activate ~callback:begin fun () ->
    Gaux.may ~f:browser#editor#save (browser#editor#get_page Oe.Page_current)
  end;
  ...
  ignore (save_as#connect#activate ~callback:begin fun () ->
    Gaux.may ~f:browser#editor#dialog_save_as (browser#editor#get_page Oe.Page_current)
  end);
  ...
  let callback _ =
    let may_current_page f = Gaux.may (browser#editor#get_page Oe.Page_current) ~f in
    try
      begin
        ...
        let filename =
          match browser#editor#get_page Oe.Page_current with
            | None -> ""
        ...
        if List.length browser#editor#file_history.File_history.content > 0 then begin
          let dialog_find_file = GMenu.menu_item ~label:"Select File..." ~packing:open_recent_menu#add () in
  ...
  let callback _ =
    try
      ...
      let has_current_page = (browser#editor#get_page Oe.Page_current) <> None in
...
let edit ~browser ~group ~flags ~get_menu_item_undo ~get_menu_item_redo () =
  let edit = GMenu.menu_item ~label:"Edit" () in
  let may f = match browser#editor#get_page Oe.Page_current with
    | None -> ()
  ...
  complet#connect#activate ~callback:(fun () -> may Module_browser_compl.create);
...
let search ~browser ~group ~flags () =
  let may f = match browser#editor#get_page Oe.Page_current with
    | None -> ()
    | Some current -> f current
  in
  ...
  ignore (find_references#connect#activate ~callback:begin
    Activity.wrap Activity.Annot begin fun () ->
      may begin fun page ->
        browser#editor#find_references (page#buffer#get_iter `INSERT)
      end
    end
  end);
...
let view ~browser ~group ~flags ~menu_item_view_menubar ~menu_item_view_toolbar ~menu_item_view_tabbar ~menu_item_view_outline~menu_item_view_messages () =
  let may f = match browser#editor#get_page Oe.Page_current with
    | None -> ()
    | Some current -> f current
  in
  ...
  let select_in_outline = GMenu.menu_item ~label:"Select in Structure Pane" ~packing:menu#add () in
  ignore (select_in_outline#connect#activate ~callback:(fun () ->
    may (fun page -> Gaux.may page#outline ~f:(fun ol -> ol#select ?align:None (page#buffer#get_mark `INSERT)))));
...
let project ~browser ~group ~flags () =
...
(*              items_build := (GMenu.separator_item ~packing:(menu#insert ~pos:5) ()) :: !items_build;*)
  ...
  let project_comp_file = GMenu.image_menu_item ~packing:menu#add () in
  let label_comp_file = GMisc.label ~xalign:0.0 ~packing:project_comp_file#add () in
...
let tools ~browser ~group ~flags () =
  ...
  module_browser#connect#activate ~callback:(fun () ->
    Module_browser_tool.create ~project:browser#current_project);
  ...
  let callback () =
    begin
      ...
      with No_current_project -> begin
        ...
        module_browser#misc#set_sensitive false;
...
let window ~browser ~group ~flags ~get_menu_item_nav_history_backward ~get_menu_item_nav_history_forward ~get_menu_item_nav_history_last () =
  ...
  let callback () =
    ...
    let bindings = List.map begin fun p ->
      ...
      let is_current = match browser#editor#get_page Oe.Page_current with
        | None -> (fun _ -> false)
        | Some current -> (fun x -> x = current)
      in
      match browser#editor#get_page (Oe.Page_file file) with
        | None -> (fun () -> ())
        | Some page ->
...
let help ~browser ~group ~flags () =
  ...
  if try int_of_string (List.assoc "debug" Oe_config.ocamleditor_param) >= 1 with Not_found -> false then begin
    ...
    ignore (crono#connect#activate ~callback:begin fun () ->
      Printf.printf "Undo/redo stack length:\n-----------------------\n%!" ;
      List.iter begin fun p ->
        let l1, l2 = (p#buffer :> Text.buffer)#undo#length in
        Printf.printf "%-80s: %4d, %4d\n%!" p#get_filename l1 l2;
      end browser#editor#pages;
      Prf.print()
    end);
  end;
  let _ = GMenu.separator_item ~packing:menu#add () in
...
let create
  ...
  file ~browser ~group ~flags;
  edit ~browser ~group ~flags ~get_menu_item_undo ~get_menu_item_redo;
  search ~browser ~group ~flags;
  view ~browser ~group ~flags ~menu_item_view_menubar ~menu_item_view_toolbar ~menu_item_view_tabbar ~menu_item_view_outline ~menu_item_view_messages;
  project ~browser ~group ~flags;
  tools ~browser ~group ~flags;
  window ~browser ~group ~flags ~get_menu_item_nav_history_backward ~get_menu_item_nav_history_forward ~get_menu_item_nav_history_last;
  help ~browser ~group ~flags;
