...
class widget ?packing () =
  ...
  let entry_list = Entry_list.create ~height:100 ~packing:vbox#pack () in
...
object (self)
  ...
  initializer
    entry_list#connect#changed ~callback:changed#call;
    radio_prepend#connect#pressed ~callback:(fun () -> replace_changed#call false);
    radio_replace#connect#pressed ~callback:(fun () -> replace_changed#call true);
    ...
    button_remove#connect#clicked ~callback:entry_list#remove_selected;
    button_select#connect#clicked ~callback:begin fun () ->
      ...
      button_all#connect#clicked ~callback:(fun () -> checklist#iter_set (fun c n -> (true, n)));
      button_none#connect#clicked ~callback:(fun () -> checklist#iter_set (fun c n -> (false, n)));
      button_ok#connect#clicked ~callback:begin fun () ->
        let entries = ref [] in
        checklist#iter begin fun checked entry ->
          if checked then (entries := entry :: !entries)
        end;
