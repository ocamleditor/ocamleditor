...
let _ =
  ...
  List.iter (fun x -> g (string_of_type x)) [Flag_Set; Flag_Clear; Bool; String]
;;
...
let _ =
  ...
  List.iter (fun x -> g (string_of_pass x)) [`key; `value; `key_value]
;;
...
class widget ~project ?packing () =
  ...
  let rend_def_value  = GTree.cell_renderer_text [`XALIGN 0.5; `EDITABLE true] in
  ...
  let _               = vc_opt_def#pack ~expand:true rend_def_check in
...
object (self)
  ...
  initializer
    ...
    ignore (rend_def_check#connect#toggled ~callback:begin fun path ->
      let row = model#get_iter path in
      model#set ~row ~column:col_opt_def_check (not (model#get ~row ~column:col_opt_def_check));
    end);
    ignore (rend_def_bool#connect#edited ~callback:begin fun path text ->
      let row = model#get_iter path in
      model#set ~row ~column:col_opt_def_value text;
    end);
    ignore (rend_def_value#connect#edited ~callback:begin fun path text ->
      let row = model#get_iter path in
      model#set ~row ~column:col_opt_def_value text;
    end);
    (*  *)
    ...
    (* Cell Data Func *)
    vc_opt_def#set_cell_data_func rend_def_check begin fun _ row ->
  ...
  method private is_flag row =
    let arg_type = model#get ~row ~column:col_opt_type in
    Str.first_chars arg_type 4 = "Flag"
  ...
  method private update_pass row =
    ...
    if is_flag then (model#set ~row ~column:col_opt_pass (string_of_pass `key));
    ...
    List.iter begin fun arg ->
      ...
      begin
        match arg.bsa_default with
          | `flag x -> model#set ~row ~column:col_opt_def_check x;
    ...
    model#foreach begin fun path row ->
      match self#find_task_by_name (model#get ~row ~column:col_opt_et_name) with
        | Some bc_et ->
          ...
          let bsa_default =
            match bsa_type with
              | Flag_Set | Flag_Clear -> `flag (model#get ~row ~column:col_opt_def_check)
