...
module App_config = struct let (//) = Filename.concat let (!!) = Filename.dirname let application_param = try List.fold_left begin fun acc x -> match Str.split (Str.regexp "=") x with | n :: v :: [] -> (n, v) :: acc | n :: [] -> (n, "") :: acc | _ -> acc end [] (Str.split (Str.regexp ",") (Sys.getenv "OCAMLEDITORPARAM")) with Not_found -> [];; let application_debug = try (List.assoc "debug" application_param) = "2" with Not_found -> false;; let application_pixmaps = let share_pixmaps = "share" // "pixmaps" in let base = !! (!! Sys.executable_name) in let path = if base = "." then ".." // share_pixmaps else base // share_pixmaps in let install_path = (!! (!! Sys.executable_name)) // share_pixmaps // "ocamleditor" in if Sys.file_exists install_path then install_path else path end
...
let arg_4_prefix = ref "/usr/local"

let cmd_line_args = [
  `Uninstall, [
    "-prefix", Set_string arg_4_prefix,
      (" Uninstallation prefix (Unix only) [default: " ^ !arg_4_prefix ^ "]");
  ];
  `Install, [
...
let external_tasks = [
  ...
  2, (fun command -> {
    ...
    et_cmd                   = "ocaml";
    et_args                  = [true,"tools/prepare_build.ml"; 
                                command = `Build && !arg_1_use_modified_gtkThread, "-use-modified-gtkThread"; 
                                command = `Build, (sprintf "-record-backtrace %b" !arg_0_record_backtrace)];
    et_phase                 = Some Before_compile;
    et_always_run_in_project = false;
    et_always_run_in_script  = true;
  });
  ...
  8, (fun command -> {
    ...
    et_args                  = [true,"tools/uninstall.ml"; 
                                command = `Uninstall, (sprintf "-prefix %S" !arg_4_prefix)];
    et_phase                 = Some Before_clean;
    et_always_run_in_project = false;
    et_always_run_in_script  = false;
  });
...
let targets = [
  ...
  "ocamleditor-bytecode", {
    ...
    package              = "compiler-libs,lablgtk2,lablgtk2.rsvg,str";
  ...
  "ocamleditor-win32", {
    ...
    package              = "compiler-libs,lablgtk2,lablgtk2.rsvg,str";
    search_path          = "+ocamldoc +xml-light gmisclib common icons otherwidgets oebuild "; (* -I *)
    required_libraries   = "resource.obj ocamlcommon dynlink odoc_info xml-light gmisclib common icons otherwidgets oebuildlib ocamleditor_lib";
    compiler_flags       = "-w syxm -g";
    linker_flags         = "-g";
    thread               = true;
    vmthread             = false;
  ...
  "ocamleditor", {
    ...
    package              = "compiler-libs,lablgtk2,lablgtk2.rsvg,str";
    search_path          = "+ocamldoc +xml-light gmisclib common icons otherwidgets oebuild "; (* -I *)
    required_libraries   = "ocamlcommon dynlink odoc_info xml-light gmisclib common icons otherwidgets oebuildlib ocamleditor_lib";
    compiler_flags       = "-w syxm -g";
    linker_flags         = "-g";
    thread               = true;
    vmthread             = false;
